...
export const openclawGuide: ToolGuide = {
  id: 'openclaw',
  name: 'OpenClaw',
  icon: '',
  providers: ['OpenAI'],
  byok: {
    ...
    steps: (apiKey) => [
      ...
      { label: '安装 OpenClaw CLI', command: 'npm install -g openclaw' },
      { label: '设置 OpenAI API Key', command: `export OPENAI_API_KEY="${apiKey}"` },
      { label: '启动 OpenClaw', command: 'openclaw' },
    ],
  },
  platform: {
    label: '平台额度',
