...
export const opencodeGuide: ToolGuide = {
  id: 'opencode',
  name: 'Opencode',
  icon: '',
  ...
  byok: {
    ...
    steps: (apiKey) => [
      ...
      { label: '安装 Opencode CLI', command: 'npm install -g opencode' },
      { label: '设置 Anthropic API Key', command: `export ANTHROPIC_API_KEY="${apiKey}"` },
      { label: '设置 OpenAI API Key（可选）', command: 'export OPENAI_API_KEY="sk-..."' },
      { label: '启动 Opencode', command: 'opencode' },
    ],
  },
  platform: {
    label: '平台额度',
